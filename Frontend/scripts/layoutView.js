...
class LayoutView {
    ...
    addToClassList(dataId, className) {
        let element = className === "highlight" 
            ? container.querySelector(`#${dataId} > div`) 
    ...
    showNewImage(srcData, id, width, height, shortName) {
    ...
    updateTileColor(tileObj, imgBase64) {
        const tile = container.querySelector(`#${tileObj.id}`);
        tile.style.backgroundImage = `url("${imgBase64}")`;        
    }
}