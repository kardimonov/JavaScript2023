class ProjectModel {
    ...
    downloadProject = async(id) => {
        ...
        if (res.images && res.images.length > 0) {
            for (const image of res.images) {
                let imageModel = new ImageModel(image.id, image.projectId);
                imageModel.name = image.name;
                imageModel.height = image.height;
                imageModel.width = image.width;
                imageModel.base64 = image.bytes; // to be corrected in the next version
                this.images.push(imageModel);
                let shortName = imageModel.name.split('.').slice(0, -1).join('.');
                this.view.showNewImage(imageModel.base64, imageModel.id, imageModel.width, imageModel.height, shortName);
            }
        }        
        ...
        for (const wall of res.walls) {
            ...
            for (const tile of wall.tiles) {
                ...
                tileModel.posXY = tile.posY;
                tileModel.imageId =tile.imageIg ?? "";
    ...
    moveTileLeft(dataId) {
        ...
        let tiles = this.currentWall.tiles.filter(i => (tile.posX > i.posX + i.width - 1) 
            && (i.posY >= tile.posY && i.posY <= tile.posY + tile.height 
            || tile.posY >= i.posY && tile.posY <= i.posY + i.height 
            || tile.posY + tile.height >= i.posY && tile.posY + tile.height <= i.posY + i.height));
        if (tiles.length == 0) {
            tile.posX = 0;
        }
    ...
    moveTileUp(dataId) {
        ...
        let tiles = this.currentWall.tiles.filter(i => (tile.posY > i.posY + tile.height - 1) 
            && (i.posX >= tile.posX && i.posX <= posX2 
            || tile.posX >= i.posX && tile.posX <= i.posX + i.width 
            || posX2 >= i.posX && posX2 <= i.posX + i.width));
        if (tiles.length == 0) {
            tile.posY = 0;
        }
        else if (tiles.length == 1) {
    ...
    processFile(file) {
        ...
        reader.onload = (event) => {            
            ...
            image.onload = (e) => {
                ...
                context.view.showNewImage(imageModel.base64, imageModel.id, imageModel.width, imageModel.height, shortName);
            }
        }        
    }
    paintTile(tileId, imageId) {
        ...
        this.view.updateTileColor(tile, image.base64);
    }
